...
function CemberinAlani(r2, piSayisi) {
  return piSayisi * Math.pow(r2, 2);
}
...
besyuzdenkucuksayilar = sayilar.filter(
  (dusukparametresi) => dusukparametresi < 500
);
...
for (let sayi in temp)
  if (temp[sayi] == 1) delete temp[sayi];
  else
    tekraredensayilar = [
      ...tekraredensayilar,
      `${sayi} sayısı ${temp[sayi]} kere tekrar edilmiştir`,
    ];
